...
func newFoo(name string, replicas *int32) *controller.Foo {
	return &controller.Foo{
		TypeMeta: metav1.TypeMeta{APIVersion: controller.SchemeGroupVersion.String()},
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: metav1.NamespaceDefault,
		},
		Spec: controller.FooSpec{
...
func (f *fixture) newController() (*Controller, informers.SharedInformerFactory, kubeinformers.SharedInformerFactory) {
	...
	c := NewController(f.kubeclient, f.client,
		k8sI.Apps().V1().Deployments(), i.Controller().V1().Foos())
	...
	for _, f := range f.fooLister {
		i.Controller().V1().Foos().Informer().GetIndexer().Add(f)
	}
...
func (f *fixture) expectUpdateFooStatusAction(foo *controller.Foo) {
...
func getKey(foo *controller.Foo, t *testing.T) string {
